...
use crate::types::{
    Count, Index, InteractionRes, Key, ReturnValue, StateInteration, StateRef, UTimeout , Value,
};
...
pub async fn list_interact(list_op: ListOps, state: StateRef) -> InteractionRes {
    match list_op {
        ...
        ListOps::BLPop(key, timeout) => {
            ...
            blocking_key_timeout(Box::new(bl), state_clone, key_clone, timeout).await.into()
        }
        ListOps::BRPop(key) => {
            let state_clone = state.clone();
            let key_clone = key.clone();
